...
export class LiveCypherService {
  ...
  requestFx = <ArgRecord, RequestResult>(
    fetch: (
      input: ArgRecord,
...
export const acquireDriver = ({
  ...
  pipe(
    Effect.tryCatchPromise(
      () => hasReachableServer(url),
      (error) => new Neo4jDbmsError(error)
    ),
    Effect.map(() => neo4j.driver(url, authToken, options))
  );

export const releaseDriver = (neo4jDriver: Neo4jDriver) =>
  Effect.promise(() => neo4jDriver.close());

export const neo4jDriverResource = (
  args: Neo4jConnectionArgs
): Effect.Effect<Scope.Scope, Neo4jDbmsError, Neo4jDriver> =>
  Effect.acquireRelease(acquireDriver(args), releaseDriver);
