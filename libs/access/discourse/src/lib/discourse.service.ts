...
import {
  ...
  createPostAsUserAt,
  createTopicAt,
  getOrCreateTopicAsUserAt,
  getOrCreateTopicAt,
  getTopicByExternalIdAt,
  NewDiscoursePost,
  NewDiscourseTopic,
...
export interface DiscourseService {
  readonly getSite: () => Effect.Effect<
    never,
    DiscourseApiError,
    GetSiteResponseContent
  >;
  ...
  >;
  readonly listPublicUsers: () => Effect.Effect<
    never,
    DiscourseApiError,
    Chunk.Chunk<DiscoursePublicUserItem>
  >;
  readonly getTopicByExternalId: ({
    external_id,
  }: {
    external_id: string;
  }) => Effect.Effect<
  ...
  readonly createTopicAs: (
    ...
    CreateTopicPostPMResponseContent
  >;
  readonly getOrCreateTopic: (
    post: NewDiscourseTopic
  ) => Effect.Effect<never, DiscourseApiError, GetTopicResponseContent>;
  readonly getOrCreateTopicAs: (
    effectiveUsername: string,
    post: NewDiscourseTopic
  ) => Effect.Effect<never, DiscourseApiError, GetTopicResponseContent>;
  readonly createPost: (
  ...
  readonly acceptPost: (
    ...
    never,
    ParseError | FetchError | HttpError | JsonBodyError,
    OperationSuccessResponse
  >;
  readonly getTopic: (
    id: string
  ...
  ) => Effect.Effect<never, DiscourseApiError, DiscoursePrivateUser>;
  readonly getUser: (
    username: string
  ) => Effect.Effect<never, DiscourseApiError, DiscoursePublicUser>;
  readonly getUserAdminDetails: (
    userid: number
  ) => Effect.Effect<never, FetchError | ParseError, DiscoursePrivateUser>;
...
const requestFx = <Args, ResponseData>(
  fetch: (
    arg: Args,
    init?: RequestInit | undefined
  ) => Promise<ApiResponse<ResponseData>>,
  arg: Args
): Effect.Effect<never, FetchError, ResponseData> => {
  return pipe(
...
const requestJson = <R>(input: any, init: RequestInit | undefined) =>
  pipe(
    request(input, init),
    Effect.map(jsonBody),
    Effect.map((x) => x as R)
...
const configureDiscourseService = (
  api: DiscourseApiConfiguration
): Effect.Effect<never, never, DiscourseService> => {
  ...
  return Effect.sync(() => ({
    getSite: (): Effect.Effect<
      never,
      DiscourseApiError,
      GetSiteResponseContent
    > => requestFx(discourse.path('/site.json').method('get').create(), {}),
    listCategories: (): Effect.Effect<
      never,
      DiscourseApiError,
      Chunk.Chunk<DiscourseCategory>
    > =>
      pipe(
        requestFx(discourse.path('/categories.json').method('get').create(), {
    ...
    listCategoryTopics: discourse
      .path('/c/{slug}/{id}/none.json')
      .method('get')
      .create(),
    listSubcategoryTopics: discourse
      .path('/c/{category}/{subcategory}/{id}.json')
      .method('get')
      .create(),
    listGroups: discourse.path('/groups.json').method('get').create(),
    listTags: discourse.path('/tags.json').method('get').create(),
    listPublicUsers: (): Effect.Effect<
      never,
      DiscourseApiError,
      Chunk.Chunk<DiscoursePublicUserItem>
    > =>
      pipe(
        requestFx(
          discourse.path('/directory_items.json').method('get').create(),
          { period: 'all', order: 'post_count' }
        ),
    ...
    getTopic: (
      ...
      requestFx(discourse.path('/t/{id}.json').method('get').create(), { id }),
    ...
    getUser: (
      ...
      pipe(
        requestFx(discourse.path('/u/{username}.json').method('get').create(), {
          username,
        }),
    ...
    getUserAdminDetails: (userid: number) =>
      pipe(
        requestFx(
    ...
    search: () =>
      requestJson<SearchResponseContent>(
        discourse.path('/search.json').method('get').create(),
        {}
      ),
  }));
};
