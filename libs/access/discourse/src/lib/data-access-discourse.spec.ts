...
describe('dataAccessDiscourse', () => {
  ...
  describe('discourseAPI', () => {
    const discourse = discourseAPI({
      ...
      apiKey:
        process.env.NX_DISCOURSE_API_KEY || 'Missing NX_DISCOURSE_API_KEY',
      apiUsername:
    ...
    it.skip('listCategories', async () => {
      const { data } = await discourse.listCategories({
    ...
    it.skip('listTags', async () => {
      ...
      const xs = pipe(data.tags);
      // console.log(xs)
      expect(xs).toContain('cypher');
    });

    it.skip('listUsers', async () => {
      const { data } = await discourse.listUsers({
        flag: 'active',
        order: 'created',
        asc: 'true',
        page: 1,
    ...
    it.skip('listUsersPublic', async () => {
      const { data } = await discourse.listUsersPublic({
        period: 'all',
        order: 'post_count',
        page: 0,
      });
    ...
    it.skip('listCategoryTopics', async () => {
      const { data } = await discourse.listCategoryTopics({
