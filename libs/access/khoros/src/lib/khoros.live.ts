...
import {
  GetAllMessagesArgs,
  GetUserByIdArgs,
  GetUsersByIdRangeArgs,
  GetMessagesOnDateArgs,
  KhorosApiConfig,
  KhorosService,
  GetMessagesWithCursor,
  GetMessageOnDateWithCursor,
  KhorosError,
} from './khoros.service';
...
export const itemsFromResult = (result: QueryASingleCollection) =>
  pipe(result.data.items, Chunk.fromIterable);

export const makeAuthorizedService = (
  auth: KhorosServiceAuthorization
): KhorosService => {
  ...
  const query = <RequestResult>(args: { q?: string }) => {
    const schedule: Schedule.Schedule<never, FetchError, number> = pipe(
  ...
  const getLatestMessages = () =>
    pipe(
      ...
      Effect.flatMap((a) =>
        a.data.size > 0
          ? Effect.succeed(Chunk.fromIterable(a.data.items))
          : Effect.fail(new KhorosError('No latest messages'))
      )
    );

  const getMessagesOnDate = ({ day }: GetMessagesOnDateArgs) =>
    pipe(
      query<QueryASingleCollection>({
        q: `SELECT * FROM messages WHERE post_time >= ${getTime(
          startOfDay(day)
        )} AND post_time <= ${getTime(endOfDay(day))} LIMIT 1000`,
      }),
  ...
  const getTagsForMessage = (messageId: string) =>
    pipe(
      query<QueryASingleCollection>({
  ...
  const getCustomTagsForMessage = (messageId: string) =>
    pipe(
      query<QueryASingleCollection>({
  ...
  const getLabelsForMessage = (messageId: string) =>
    pipe(
      query<QueryASingleCollection>({
  ...
  const getKudosForMessage = (messageId: string) =>
    pipe(
      query<QueryASingleCollection>({
  ...
  const getTopicsOnDate = ({ day }: GetMessagesOnDateArgs) =>
    pipe(
      query<QueryASingleCollection>({
        q: `SELECT * FROM messages WHERE depth = 0 AND post_time >= ${getTime(
          startOfDay(day)
        )} AND post_time <= ${getTime(endOfDay(day))} LIMIT 1000`,
      }),
      Effect.map((a) => Chunk.fromIterable(a.data.items))
  ...
  const getMessagesOnDateWithCursor = ({
    day,
    cursor,
    limit,
  }: GetMessageOnDateWithCursor) =>
    pipe(
      query<QueryASingleCollection>({
        q: `SELECT * FROM messages WHERE post_time >= ${getTime(
          startOfDay(day)
        )} AND post_time <= ${getTime(endOfDay(day))} ${optionalLimitLiQL(
          limit
        )} ${optionalCursorLiQL(cursor)}`,
      }),
      Effect.map((a) => ({ items: a.data.items, cursor: a.data.next_cursor }))
    );

  const getAllMessagesOnDate = ({ day }: { day: Date }) =>
    pipe(
      Effect.unfold('' as string | undefined, (cursor) =>
        pipe(
          ...
          Effect.map((page) =>
            Chunk.size(page.items) > 0
              ? Option.some([page.items, page.cursor])
              : Option.none()
          )
        )
      ),
  ...
  const optionalCursorLiQL = (cursor?: string) =>
    cursor !== undefined && cursor.length > 1 ? `CURSOR '${cursor}'` : '';

  const optionalLimitLiQL = (limit = 1000) =>
    limit !== undefined ? `LIMIT ${limit}` : '';

  const getMessagesWithCursor = ({ cursor, limit }: GetMessagesWithCursor) =>
    pipe(
      query<QueryASingleCollection>({
        q: `SELECT * FROM messages ORDER BY post_time ASC ${optionalLimitLiQL(
          limit
        )} ${optionalCursorLiQL(cursor)} `,
      }),
      Effect.map((a) => ({ items: a.data.items, cursor: a.data.next_cursor }))
        ...
        pipe(
          ...
          Effect.map((page) =>
            page.cursor !== undefined
              ? Option.some([page.as, page.cursor])
              : Option.none()
          )
        )
      )
  ...
  const getUserById = ({ id }: GetUserByIdArgs) =>
    pipe(
      query<QueryASingleCollection>({
  ...
  const getUsersWithinRange = ({ from, to }: GetUsersByIdRangeArgs) =>
    pipe(
      query<QueryASingleCollection>({
      ...
      Effect.map((result) =>
        result.data.size > 0
          ? Chunk.fromIterable(result.data.items)
          : Chunk.empty<Item>()
      )
    );
  ...
  const getAllUserIDs = () =>
    pipe(
      Effect.unfold('', (cursor) =>
        pipe(
          query<QueryASingleCollection>({
            q: `SELECT id FROM users ORDER BY id ASC ${optionalLimitLiQL(
              1000
            )} ${optionalCursorLiQL(cursor)}`,
          }),
          Effect.map((result) =>
            result.data.size > 0 && result.data.next_cursor !== undefined
              ? Option.some([
                  Chunk.fromIterable(result.data.items),
