...
export const equivalentByItemId = Equivalence.make<Pick<Item, 'id'>>(
  (self, that) => that.id === self.id
);

export const dedupeBySsoId = (chunks: Chunk.Chunk<KhorosAuthor>) =>
  pipe(
    chunks,
...
export const orderItemsByMessageDepth = Order.make<Item>((self, that) =>
  compareNumbers(that.depth || 0)(self.depth || 0)
);

export const sortItemsByMessageDepth = Chunk.sort(orderItemsByMessageDepth);

export const dedupeById = (chunks: Chunk.Chunk<Pick<Item, 'id'>>) =>
  pipe(
    chunks,
    Chunk.toReadonlyArray,
