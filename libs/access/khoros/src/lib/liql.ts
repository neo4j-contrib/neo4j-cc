 ...
 */
export const withOptionalCursor = (cursor?: string) =>
  cursor !== undefined ? `CURSOR '${cursor}` : '';

export const select = (collection: LiCollectionName) =>
...
export const getTopicsOnBoard = (board_id: string, cursor?: string) =>
  `SELECT * FROM messages WHERE depth = 0 AND board.id = '${board_id}' ${withOptionalCursor(
    cursor
  )}`;
