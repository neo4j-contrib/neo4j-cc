...
describe('LiveKhorosService', () => {
  const live = makeLiveKhorosService({
    baseUrl: KHOROS_API_URL,
    login: KHOROS_LOGIN,
    password: KHOROS_PASSWORD,
  });
  ...
  it('can search effectfully', async () => {
    const result = await pipe(
      ...
      Effect.flatMap((khoros) =>
        khoros.query<QueryASingleCollection>({
  ...
  it.skip(
    ...
    async () => {
      ...
      const result = await pipe(
        Effect.Do(),
        Effect.bind('khoros', () => live),
        Effect.bind('page1', ({ khoros }) =>
          khoros.getMessagesWithCursor({ limit: resultLimit })
        ),
        Effect.bind('page2', ({ khoros, page1 }) =>
          khoros.getMessagesWithCursor({ cursor: page1.cursor })
        ),
        Effect.runPromise
      );
  ...
  it('can get tags for a message', async () => {
    const result = await pipe(
      Effect.Do(),
      Effect.bind('khoros', () => live),
      Effect.bind('messageWithTags', ({ khoros }) =>
        pipe(khoros.getMessageById('64247'))
      ),
      Effect.flatMap(({ khoros, messageWithTags }) =>
        khoros.getTagsForMessage(messageWithTags.id)
      ),
      Effect.runPromise
    );
  ...
  it('can get labels for a message', async () => {
    const result = await pipe(
      ...
      Effect.bind('khoros', () => live),
      Effect.bind('messageWithTags', ({ khoros }) =>
        pipe(khoros.getMessageById('64017'))
      ),
      Effect.flatMap(({ khoros, messageWithTags }) =>
        khoros.getLabelsForMessage(messageWithTags.id)
      ),
      Effect.runPromise
    );
