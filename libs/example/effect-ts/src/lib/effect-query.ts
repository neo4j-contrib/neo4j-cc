...
export const userNames: ReadonlyMap<number, string> = new Map(
  pipe(
    ...
    ReadonlyArray.zipWith(
      pipe(
        ReadonlyArray.range(97, 122),
        ReadonlyArray.map((a) => String.fromCharCode(a))
      ),
      (a, b) => [a, b] as const
    )
  )
...
export interface GetAllIds
  extends Request.Request<never, ReadonlyArray<number>> {
...
export const UserDataSource: DataSource.DataSource<never, UserRequest> =
  DataSource.makeBatched('UserRequestDataSource', (requests) =>
    pipe(
      Effect.dieMessage('Duplicate requests'),
      Effect.when(
        () =>
          HashSet.size(HashSet.fromIterable(requests)) !== Chunk.size(requests)
      ),
      Effect.zipRight(
...
export const getAllUserIds: Query.Query<
  never,
  never,
  ReadonlyArray<number>
> = Query.fromRequest(GetAllIds({}), UserDataSource);

export const getUserNameById = (
  id: number
): Query.Query<never, never, string> =>
  Query.fromRequest(GetNameById({ id }), UserDataSource);

export const getAllUserNames: Query.Query<
  never,
  never,
