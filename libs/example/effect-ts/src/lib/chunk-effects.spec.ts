...
describe('Chunk', () => {
  it('unfold', async () => {
    const result = Chunk.unfold(0, (s) =>
      s < 4 ? Option.some([`#${s}`, s + 1]) : Option.none()
    );
    expect(Chunk.toReadonlyArray(result)).toStrictEqual([
      '#0',
      '#1',
      '#2',
      '#3',
...
describe('Effect chunks', () => {
  it('unfold', async () => {
    const result = pipe(
      Effect.unfold(0, (s) =>
        s < 4
          ? Effect.succeed(Option.some([`#${s}`, s + 1]))
  ...
  it('can be sequential', async () => {
    ...
    const result = await pipe(
      iterable,
      Effect.forEach(effectfulIncrement),
      Effect.runPromise
    );
  ...
  it('can be unbounded parallel', async () => {
    ...
    const result = await pipe(
      iterable,
      Effect.forEachPar(effectfulIncrement),
      Effect.runPromise
    );
    expect(result).toStrictEqual(Chunk.fromIterable([2, 3, 4]));
  });
