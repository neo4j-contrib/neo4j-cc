...
export interface FileSystemService {
  readonly write: (args: FileWithContent) => Effect.Effect<never, FileSystemError, void>
  readonly exists: (args: FileLocation) => Effect.Effect<never, FileSystemError, ExistsResult>
}

export const FileSystemService = Context.Tag<FileSystemService>()

const catchReasonInFileSystemError = (reason:unknown) => new FileSystemError(reason)
...
export const makeLocalFileSystemService = ({path, criteria}:FileSystemConfiguration) => {
