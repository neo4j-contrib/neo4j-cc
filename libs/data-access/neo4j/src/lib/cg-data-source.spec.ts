...
const getAllUserNames = getAllUserIds['|>'](
...
describe('Query', () => {
  ...
  it('failure to complete request is query failure', async () => {
    ...
    expect(await T.runPromiseExit(T.untraced(f))).toEqual(
      Ex.die(
        new QueryFailure(UserRequestDataSource, new GetNameById({ id: 27 }))
      )
    );
  });
  ...
  it('allows gen syntax - NoSuchElementException', async () => {
    ...
    expect(await T.runPromiseExit(f)).toEqual(
      Ex.fail(new NoSuchElementException())
    );
  });
