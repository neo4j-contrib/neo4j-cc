...
import { FetchError, JsonBodyError, BufferBodyError, HttpError } from './http-errors'
...
export const getRetrySecondsOr = (retryAfter: string | null, otherwise: number) => pipe(
  retryAfter,
  Option.fromNullable,
...
export const isHttpTooManyRequests = (error:HttpError | FetchError): error is HttpError => (error._tag === "HttpError") && (error.response.status === 429)

export const respectHttp429 = pipe(
  Schedule.once(),
  Schedule.contramap( (error: HttpError | FetchError) => isHttpTooManyRequests(error)
    ? Schedule.fromDelay(Duration.seconds(getRetrySecondsOr(error.response.headers.get('Retry-After'), 20)))
    : Schedule.stop()
  )
)
...
export const request = <R extends Response>(input: RequestInfo, init?: RequestInit | undefined) => 
  pipe(
