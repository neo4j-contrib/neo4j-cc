import { pipe, Effect, Layer, Logger, ReadonlyArray, HashMap, Either, Option, Chunk } from '@neo4j-cc/prelude';
import { DiscourseApiConfiguration } from './data-access-discourse';
import { DiscourseApiError, DiscourseService, makeDiscourseService } from './discourse.service';
...
import { DiscoursePublicUser, DiscoursePrivateUser, DiscoursePublicUserItem } from './discourse-schemas';

runtimeDebug.traceExecutionLogEnabled = true;
runtimeDebug.defaultLogLevel = "Debug";


const pickRandomElement = <A>(users: readonly A[]):A => users[Math.floor(Math.random() * users.length)]
...
describe("DiscourseService", () => {
  ...
  const discourseLayer = Effect.provideLayer(makeDiscourseService(discourseApiConfig))

  const discourseFunctionEffect = <O>(f: (discourse: DiscourseService) => Effect.Effect<never, DiscourseApiError, O>) => pipe(
    Effect.service(DiscourseService),
    Effect.flatMap(f), 
    discourseLayer
  )
  ...
  it("createTopic", async () => {
    const createTopic = (discourse:DiscourseService) => discourse.createTopic(arbitraryTopic());
  ...
  it.skip("getOrCreateTopic for fresh topic, repeat, they should be the same", async () => {
    ...
    const result = await pipe(
      Effect.Do(),
      Effect.bind("discourse", () => Effect.service(DiscourseService)),
      Effect.bind("topicDetail", () => Effect.succeed(arbitraryTopic(`# getOrCreateTopic\nlike an upsert without transactional safety uniquely :)`))),
      // Effect.bind("topicDetail", () => Effect.succeed(arbitraryTopic())),
      Effect.bind("freshTopic", ({discourse, topicDetail}) => discourse.getOrCreateTopic(topicDetail)),
  ...
  it.skip("createTopic debug", async () => {
    const debugTopic = {
      "title":"Non Interactive install is not working while install neo4j enterprise edition",
      "external_id":"khoros_62793",
      "created_at":"2022-12-02T11:00:19.096-08:00",
      "embed_url":"https://community.neo4j.com/t5/drivers-stacks/non-interactive-install-is-not-working-while-install-neo4j/td-p/62793",
    ...
    const createTopic = (discourse:DiscourseService) => discourse.createTopic(debugTopic);
  ...
  it.skip("find non-existent topic by external_id", async () => {
    const createTopic = (discourse:DiscourseService) => discourse.getTopicByExternalId({external_id: 'no-such-external-id'});
  ...
  it("getUser", async () => {
    ...
    const getAuthor = (discourse:DiscourseService) => pipe(
      Effect.Do(),
  ...
  it("getUser at random", async () => {
    ...
    const getAuthor = (discourse:DiscourseService) => pipe(
      ...
      Effect.flatMap(({discourse, users}) => 
        (discourse.getUser(pickRandomElement(
          Chunk.toReadonlyArray(users)
        ).user.username))
      )
    )
  ...
  it("getUserByExternalId at random", async () => {
    ...
    const getRandomByExternalId = (discourse:DiscourseService) => pipe(
      Effect.Do(),
      Effect.bind("discourse", () => Effect.succeed(discourse)),
      Effect.bind("users", ({discourse}) => discourse.listPublicUsers()),
      Effect.bind("randomUser", ({users}) =>  Effect.succeed(pickRandomElement(Chunk.toReadonlyArray(users)))),
      Effect.bind("randomUserDetails", ({randomUser}) => discourse.getUserAdminDetails(randomUser.id)),
      Effect.flatMap(({discourse, randomUserDetails}) => discourse.getUserByExternalId(randomUserDetails.single_sign_on_record.external_id))
    )
  ...
  it("post as random user", async () => {
    ...
    const randomUserPost = (discourse:DiscourseService) => pipe(
      Effect.Do(),
      Effect.bind("discourse", () => Effect.succeed(discourse)),
      Effect.bind("users", ({discourse}) => discourse.listPublicUsers()),
      Effect.bind("randomUser", ({discourse, users}) => discourse.getUser(pickRandomElement(Chunk.toReadonlyArray(users)).user.username)),
      Effect.flatMap(({discourse, randomUser}) => discourse.createTopicAs(randomUser.username, arbitraryTopic()))
    )
