...
export const createTopicAsUserAt = (api:DiscourseApiConfiguration) => {
  ...
  const createTopicAs = (effectiveUsername: string, post:NewDiscourseTopic) => {
...
export const getTopicByExternalIdAt = (api:DiscourseApiConfiguration):({ external_id }: {
  external_id: string;
}) => Effect.Effect<never, DiscourseApiError, Option.Option<GetTopicResponseContent>> => {
  ...
  const getTopicByExternalId = ({external_id}:{external_id:string}):Effect.Effect<never, DiscourseApiError, Option.Option<GetTopicResponseContent>> => {
    ...
    return pipe(
      ...
      Effect.flatMap(Option.match(
        () => pipe(
          ...
          Effect.flatMap(_ => getTopicByExternalId({external_id: post.external_id})),
          Effect.flatMap(Option.match(
            () => Effect.fail(new DiscourseServiceError(`failed to create topic for ${post.external_id}`, post)),
...
export const getOrCreateTopicAsUserAt = (api:DiscourseApiConfiguration) => {
  ...
  const getOrCreateTopic = (effectiveUsername: string, post:NewDiscourseTopic) => {
    ...
    return pipe(
      getTopicByExternalId({external_id: post.external_id}),
      Effect.flatMap(Option.match(
        () => pipe( // topic not found, so create it!
          request(`${api.baseUrl}/posts.json`, requestOptions),
          Effect.catchTag("HttpError", (error) => 
            Effect.fail(new DiscourseServiceError(`failed to create post ${post.external_id} as ${effectiveUsername}`, {error:error.response, headers: headersAsRecord(error.response.headers), body: raw}))
          ),
          Effect.flatMap(jsonBody),
          Effect.map(body => body as CreateTopicPostPMResponseContent),
          Effect.flatMap(_ => getTopicByExternalId({external_id: post.external_id})),
...
export const createPostAsUserAt = (api:DiscourseApiConfiguration) => {
  ...
  const createPost = (effectiveUsername: string, post:NewDiscoursePost) => {
    ...
    return pipe(
      request(`${api.baseUrl}/posts.json`, requestOptions),
      Effect.catchTag("HttpError", (error) => 
        Effect.fail(new DiscourseServiceError(`failed to create post ${post.topic_id} as ${effectiveUsername}`, {error:error.response, headers: headersAsRecord(error.response.headers), body: raw}))
      ),
      Effect.flatMap(jsonBody),
      Effect.map(body => body as CreateTopicPostPMResponseContent),
    )
