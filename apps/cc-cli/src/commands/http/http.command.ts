...
import * as Http from '@neo4j-cc/access-http';
...
const todo = pipe(
  ...
  Effect.tap((todo) =>
    Effect.succeed(console.log(`Todo: ${JSON.stringify(todo)}`))
  ),
  Effect.catchTag('FetchError', (error) =>
    Effect.logError(`FetchError: ${JSON.stringify(error)}`)
  ),
  Effect.catchTag('JsonBodyError', (error) =>
    Effect.logError(`JsonBodyError: ${JSON.stringify(error)}`)
  )
);

const commandDo = (url: string) =>
  pipe(
    Effect.Do(),
...
const makeCliServiceFromToolbox = (toolbox: Toolbox) =>
  Layer.effect(
    CliService,
    Effect.sync(() => ({
      toolbox,
