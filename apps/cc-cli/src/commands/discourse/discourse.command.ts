...
import { FetchError, jsonBody, JsonBodyError } from '@neo4j-cc/access-http';
...
const getCategories = ({ discourse }: DiscourseCommandArgs) =>
  pipe(
    discourse.listCategories(),
    // Effect.map(response => response.category_list.categories),
    Effect.map(
      Chunk.map((category) => [category.id, category.name] as [number, string])
    ),
...
const listCategories = pipe(
  ...
  Effect.bind('categoryIdToNameMap', ({ discourse }) =>
    getCategories({ discourse })
  ),
  Effect.tap(({ categoryIdToNameMap }) => {
    console.log(JSON.stringify(HashMap.values(categoryIdToNameMap)));
...
const handler = (argv: CommandOptions) =>
  pipe(
    listCategories,
    Effect.provideLayer(pipe(makeDiscourseService(toServiceConfig(argv)))),
    Effect.runPromise
  );
