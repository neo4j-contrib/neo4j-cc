...
import { discourseCategoryNumberFor, externalIdFor, khorosToSsoUserDetails, khorosToDiscourseTopic, khorosToDiscoursePost, discourseUserFromAuthor, findDiscourseTopicForKhorosMessage, findPostInTopic } from '@neo4j-cc/pipe-khoros-to-discourse';

export type SubcommandErrors = FetchError | JsonBodyError | KhorosError | HtmlParseError | FileSystemError
...
const toDiscourseConfig = (argv:CommandOptions): DiscourseApiConfiguration => ({
...
const toConsole = (s:string) => { console.log(s); return Effect.unit() }

const pathOfKhorosSsoDiscourse = (entry:[KhorosAuthor, SsoUserDetails, DiscoursePrivateUser]) => 
...
const authorsFromMessages = (messages:Chunk.Chunk<KhorosMessage>, khoros:KhorosService) => pipe(
  messages,
  Chunk.map( (message) => message.author),
  Effect.forEach( (author) => pipe(
...
const syncSsoUsers = (userPairs:Chunk.Chunk<[KhorosAuthor, SsoUserDetails]>, discourse:DiscourseService) => pipe(
  userPairs,
  Effect.forEach( khorosSsoPair => pipe(    
    Effect.log(`syncing: { login: ${khorosSsoPair[0].login}, email:${khorosSsoPair[1].email} }`),
    Effect.flatMap(() => discourse.syncSso(khorosSsoPair[1])),
    Effect.map( discourseUser => ([...khorosSsoPair, discourseUser] as [KhorosAuthor, SsoUserDetails, DiscoursePrivateUser])),
...
const doCommand = (argv:CommandOptions):Effect.Effect<KhorosService | DiscourseService | FileSystemService, unknown, void> => {
  return pipe(
    Effect.Do(),
    ...
    Effect.bind("discourse_categories", ({discourse}) => pipe(discourse.listCategories(), Effect.map(Chunk.toReadonlyArray))),
    ...
    Effect.bind("khoros_messages", ({khoros}) => pipe(
    ...
    Effect.bind("khoros_topics", ({khoros, khoros_messages}) => pipe(
      topicsFromMessages(khoros_messages, khoros),
    )),
    // Effect.tap( ({khoros_topics}) => toConsole(JSON.stringify(Chunk.toReadonlyArray(khoros_topics)))), 
    Effect.tap(({khoros_topics}) => Effect.log(`khoros_topics: ${khoros_topics.length}`)),
    Effect.bind("khoros_posts", ({khoros_messages}) => Effect.succeed(postsFromMessages(khoros_messages))),
    Effect.tap(({khoros_posts}) => Effect.log(`khoros_posts: ${khoros_posts.length}`)),
    Effect.bind("khoros_authors", ({khoros, khoros_messages, khoros_topics}) => pipe(
      authorsFromMessages(khoros_messages, khoros),
      Effect.flatMap( messageAuthors => pipe(
    ...
    Effect.bind("khoros_sync_pairs", ({khoros_authors}) => pipe(
      khoros_authors, 
      Chunk.map( (fromKhoros) => pipe(
        khorosToSsoUserDetails(fromKhoros),
        Option.map(ssoDetails => [fromKhoros, ssoDetails] as [KhorosAuthor, SsoUserDetails])
      )), 
      Chunk.compact,
      Effect.succeed
    )),
    ...
    Effect.bind("khoros_discourse_topics", ({khoros, discourse, khoros_topics, discourse_categories, khoros_sync_discourse_users}) => pipe(
      khoros_topics,
      Effect.forEach( khorosTopic => pipe(
        khoros.getTagsForMessage(khorosTopic.id),
        Effect.tap(() => Effect.log(`migrating topic ${khorosTopic.id} ${khorosTopic.post_time_friendly} for ${khorosTopic.author.login}`)),
        Effect.flatMap( tags => pipe(
        ...
        Effect.flatMap( labelsAndTags => khorosToDiscourseTopic(khorosTopic, discourseCategoryNumberFor(khorosTopic.board, discourse_categories), [ ...labelsAndTags, "khoros"])),
        Effect.map( preparedDiscourseTopic => pipe(
          discourseUserFromAuthor(khorosTopic.author, khoros_sync_discourse_users),
          ...
          Option.map( 
            ...
            foundAuthor => [foundAuthor, preparedDiscourseTopic] as [DiscoursePrivateUser, NewDiscourseTopic]
            )
        )),
        
        Effect.map(Option.map(preparedAuthorWithPost => discourse.getOrCreateTopicAs(preparedAuthorWithPost[0].username, preparedAuthorWithPost[1]))),
        Effect.flatMap(Effect.fromOption),
        Effect.flatten,
        Effect.map(Option.some),
        Effect.catchTag("None", () => Effect.succeed(Option.none)),
        Effect.map(Option.map(discourseTopic => ([khorosTopic, discourseTopic] as [KhorosMessage, GetTopicResponseContent]))),
    ...
    Effect.bind("khoros_discourse_posts", ({discourse, khoros_posts, khoros_discourse_topics, khoros_sync_discourse_users}) => pipe(
      khoros_posts,
      Chunk.filter( khorosPost => (khorosPost.author.id !== "-1")),
      Chunk.map( khorosPost => pipe(
        findDiscourseTopicForKhorosMessage(khorosPost.topic.id, khoros_discourse_topics),
        Option.map(foundTopic => ([khorosPost, foundTopic] as [KhorosMessage, GetTopicResponseContent]))
      )),
      Chunk.compact,
      Effect.forEach( ([khorosPost, matchingDiscourseTopic]) => 
        pipe(
          ...
          // Effect.flatMap( matchingDiscourseTopic => pipe(
            ...
            Effect.tap(() => Effect.log(`migrating post ${khorosPost.id} ${khorosPost.post_time_friendly} by ${khorosPost.author.login} to topic ${khorosPost.topic.id}`)),
            Effect.map(preparedPost => ([matchingDiscourseTopic, preparedPost])),
          // )),
          Effect.flatMap( ([discourseTopic, preparedDiscoursePost]) => pipe(
            discourseUserFromAuthor(khorosPost.author, khoros_sync_discourse_users),
            Option.match(
              () => Effect.fail(new DiscourseServiceError(`Author ${JSON.stringify(khorosPost.author)} not found`)),
              (foundAuthor) => Effect.succeed([discourseTopic, foundAuthor, preparedDiscoursePost] as [GetTopicResponseContent, DiscoursePrivateUser, NewDiscoursePost])
              )
          )),
          Effect.flatMap(([discourseTopic, discourseAuthor, preparedDiscoursePost]) => pipe(
            findPostInTopic(discourseTopic, preparedDiscoursePost, discourseAuthor.username),
            Option.match(
              () => pipe(
                discourse.createPostAs(discourseAuthor.username, preparedDiscoursePost), 
                Effect.map(_ => preparedDiscoursePost),
              ),
              (_) => pipe(Effect.log("post already exists"), Effect.map(_ => preparedDiscoursePost))
            ),
          )),
...
const renameCategorySlugs = (allTags: string[], discourse_categories: readonly DiscourseCategory[]): readonly string[] => pipe(
  allTags,
  ReadonlyArray.map( tag => pipe(
