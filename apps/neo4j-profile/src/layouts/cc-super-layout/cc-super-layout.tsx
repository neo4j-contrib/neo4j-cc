  ...
  return (
    <>
      ...
      <CcNavbar.Start className="grid grid-flow-col gap-2">
        ...
        {
          items.map(routable => (
            <Link to={routable.path}><CcButton size='xs' className="text-sm normal-case" color="ghost">{routable.title}</CcButton></Link>
          ))
        }
      </CcNavbar.Start>
      <CcNavbar.End>
        <CcSelect data-choose-theme size='xs' className="w-32 pr-0 m-0 border-none" color='ghost'>
          {themeNames.map( themeName => (<CcSelect.Option value={themeName}>{themeName}</CcSelect.Option>))}
        </CcSelect>
      </CcNavbar.End>
    </CcNavbar>
    ...
    <Routes>
      {
        items.map(routable => (
          <Route path={routable.path} element ={(
            <routable.Element />
          )} />
        ))
      }
